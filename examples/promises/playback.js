...
connect("http://ari.js:8088", "user", "secret")
  .then(function (ari) {
      ...
      function (event, incoming) {
        incoming
          ...
          .then(function () {
            ...
            return incoming.play({ media: "sound:demo-congrats" }, playback);
          })
    ...
    function registerDtmfListeners(playback, incoming) {
      incoming.on(
        ...
        function (event, channel) {
          ...
          switch (digit) {
            case "5":
              playback.control({ operation: "pause" }).catch(function (err) {});
              break;
            case "8":
              playback
                .control({ operation: "unpause" })
                .catch(function (err) {});
              break;
            case "4":
              playback
                .control({ operation: "reverse" })
                .catch(function (err) {});
              break;
            case "6":
              playback
                .control({ operation: "forward" })
                .catch(function (err) {});
              break;
            case "2":
              playback
              ...
              break;
            case "#":
              playback.control({ operation: "stop" }).catch(function (err) {});
              incoming.hangup().finally(function () {
                process.exit(0);
              });
              break;
            default:
